...
const Register: PageComponent = () => {
  ...
  <div className="relative bg-white w-full h-[65%] px-8 pb-5 flex flex-col justify-between items-center">
        <div>
          <h1 className="text-4xl font-bold text-[#ffad69]">{t('signup')}</h1>
          <p className="text-gray-400 mt-4 leading-relaxed">{t('signup_message')}</p>
        </div>
        ...
        >
          ...
          <button className="w-full py-3 rounded-xl bg-[#ffad69] text-white font-semibold shadow-md hover:opacity-95 transition-opacity">
            {t('signup')}
          </button>
          <p className="text-gray-400 mt-4 leading-relaxed text-center">
            {t('already_have_account')}{' '}
            <a href="/auth/login" className="text-[#090c9b]">{t('signin')}</a>
          </p>
        </form>
      </div>
    </section>
...
Register.metadata = {
  title: 's\'Cool KIT - Register',
  description: 'Register page of s\'Cool KIT application',
};
